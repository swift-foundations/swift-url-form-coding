...
extension Form {
    public final class Decoder: Swift.Decoder {
        ...
        private func unbox<T: Decodable>(_ value: Container, as type: T.Type) throws -> T {
            if type == Data.self {
                guard let result = try self.unbox(value, as: Data.self) as? T else {
        ...
        public struct DataDecodingStrategy: Sendable {
            ...
            public static func custom(
                _ strategy: @escaping @Sendable (String) -> Data?
        ...
        public struct ArrayParsingStrategy: Sendable {
            ...
            public init(
                parse: @escaping @Sendable (String) -> Container,
                handleSingleValue: Bool = false
            ) {
                self.parse = parse
            ...
            public static func custom(
                _ parseFunction: @escaping @Sendable (String) -> Container
...
extension Form.Decoder {
    ...
    @Sendable
    fileprivate static func convert(_ formData: RFC_2388.FormData) -> Container {
        switch formData {
        case .value(let str):
    ...
    @Sendable
    fileprivate static func parseUsingRFC2388(
